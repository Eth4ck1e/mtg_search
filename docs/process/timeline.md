...
The system this term builds is a **natural-language semantic search** over the ~30k unique cards in Scryfall's Oracle corpus. The research question is how to bridge the *query-document asymmetry* between short informal player queries ("cards that flicker creatures," "cheap red removal") and the formal prose of Wizards-authored card text — a gap that traditional keyword search and naive vector search both fail on. The architectural response is a **three-tower retrieval pipeline**: (1) a SQL pre-filter over structured card attributes (color, mana value, type) on Postgres + pgvector; (2) a HyDE-style query rewriter (Gao et al., 2022) that transforms the user query into a hypothetical card-text representation; (3) semantic vector search using `sentence-transformers/multi-qa-distilbert-cos-v1` inside the pre-filtered candidate set. A hand-curated 26-query evaluation set with tri-state relevance judgments (Järvelin & Kekäläinen, 2002; Sormunen, 2002) provides the measurement harness. Each retrieval-architecture change is measured against a baseline (recall@10 = 0.019, MRR = 0.154, from `experiment_runs.id = 13`) so the contribution of each component is quantified rather than asserted.
